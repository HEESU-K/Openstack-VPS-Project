...
### 4. 인프라 관측성 고도화 및 실시간 알림 체계
* **다단계 임계치 설정**: 노트북 환경의 하드웨어 가용성을 고려하여 메모리 사용량에 따른 **Warning(80%)**, **Critical(90%)** 알림을 구분하여 운영자의 선제적 대응 시간을 확보했습니다.
* **모니터링 파이프라인**: `Prometheus` → `Alertmanager` → `Discord Webhook` 체계를 구축하여 호스트 리소스뿐만 아니라 오픈스택 핵심 서비스(Nova, Neutron 등)의 가동 상태를 실시간 감지합니다.
* **장애 감지 지표 (Alert Rules)**:
    * **Resource Saturation** : RAM 사용률 90% 초과 및 루트 디스크 가용량 10% 미만 감지
    * **Service Availability** : 오픈스택 핵심 서비스 및 모니터링 에이전트(Exporter) 다운 감지
    * **System Health** : CPU 코어 수 대비 시스템 부하 지수(Load Average) 급증 및 OOM Killer 작동 이력 감지
---
- **실시간 장애 인지 및 디스코드 알림**
<img width="622" height="719" alt="스크린샷 2026-03-21 172244" src="https://github.com/user-attachments/assets/75adb00e-645c-41ee-b3f6-2a27c22732f1" />
